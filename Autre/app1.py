...
selected_date_range = slider_placeholder.slider(
...
st.subheader(f"période de recherche: {selected_date_range[0]} - {selected_date_range[1]}")
st.subheader(f"type: {type(selected_date_range[0])} - {type(selected_date_range[1])}")


##### BARRE DE RECHERCHE MANUELLE
with st.expander("🔍 Rechercher par ville ou code postal", expanded=st.session_state["lat"] is None):
